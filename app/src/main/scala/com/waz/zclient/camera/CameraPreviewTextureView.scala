...
class CameraPreviewTextureView(val context: Context, val attrs: AttributeSet, val defStyleAttr: Int) extends TextureView(context, attrs, defStyleAttr) with ViewHelper with TextureView.SurfaceTextureListener {
   ...
   */
  val isNatualOrientationLandscape = {
    val config: Configuration = context.getResources.getConfiguration
    val defaultRotation: Int = context.getSystemService(Context.WINDOW_SERVICE).asInstanceOf[WindowManager].getDefaultDisplay.getRotation

    defaultRotation match {
      case Surface.ROTATION_0 | Surface.ROTATION_180 if config.orientation == Configuration.ORIENTATION_LANDSCAPE => true
      case Surface.ROTATION_90 | Surface.ROTATION_270 if config.orientation == Configuration.ORIENTATION_PORTRAIT => true
      case _ => false
    }
  }
  ...
  val orientationListener = returning {
    new OrientationEventListener(context) {
      override def onOrientationChanged(orientation: Int) =
        controller.deviceOrientation ! Orientation(orientation, isNatualOrientationLandscape)
    }
  }(_.enable())
